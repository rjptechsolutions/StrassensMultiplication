...
c=n/2
if(n<2 and c!=0):
    print("We can not find Strassen Multiplication")
elif(n==2):
    m1 = np.random.rand(n,n)
    m2 = np.random.rand(n,n)
    print(m1)
    print(m2)
    p1=m1[:1,:1]*(m2[:1,1:2]-m2[1:2,1:2])
    p2=m2[1:2,1:2]*(m1[:1,:1]+m1[:1,1:2])
    p3=m2[:1,:1]*(m1[1:2,:1]+m2[1:2,1:2])
    p4=m2[1:2,1:2]*(m2[1:2,:1]-m2[:1,:1])
    p5=(m1[:1,:1]+m1[:1,1:2])*(m2[:1,:1]+m2[1:2,1:2])
    p6=(m2[:1,1:2]-m1[1:2,1:2])*(m2[1:2,:1]+m2[1:2,1:2])
    p7=(m1[:1,:1]-m1[1:2,:1])*(m2[:1,:1]+m2[:1,1:2])
    a11=p5+p5+p4-p2
    a12=p1+p2
    a21=p3+p4
    a22=p1+p5-p3-p7
    answer =[a11,a12,a21,a22]
    an1=np.concatenate((a11,a21), axis=0)
    an2=np.concatenate((a12,a22), axis=0)
    anf=np.concatenate((an1,an2), axis=1)
    print(anf)
else:
